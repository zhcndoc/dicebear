...
DiceBear 与 Nuxt 的通用渲染模型非常契合。头像可以在 SSR 期间于服务器端生成，
也可以在 Nitro 端点中生成，或者在普通的客户端组件中生成——选择最符合页面
[渲染模式](https://nuxt.com/docs/guide/concepts/rendering) 的方式即可。

你可以通过 [JS-Library](/how-to-use/js-library/) 或
[HTTP-API](/how-to-use/http-api/) 在 [Nuxt](https://nuxt.com/) 中使用 DiceBear。

## 使用 JS 库

### 通用组件

将生成逻辑包裹在 `computed` 中，头像就会在组件渲染的任一端生成。由于结果是 data URI，
标记在客户端进行 hydration 时不会重新运行渲染器。
...
`idRandomization` 依赖于宿主环境的非种子 RNG，因此 SSR 期间生成的 ID 与客户端重新渲染时不会匹配——Vue 会记录 hydration mismatch。对于 SSR 生成的头像，请保持 `idRandomization: false`；或者将组件包裹在 `<ClientOnly>` 中并接受视觉上的闪烁。

如果你需要同一页面上多个头像拥有唯一 ID，请整页进行服务端渲染，并跳过头像子树的客户端 hydration。

:::

### Nitro 端点

当你需要自定义缓存或种子校验时，可以通过你自己的 URL 暴露 DiceBear：
...
如果要进行按请求的 SSR 缓存（这样当多个组件请求同一个 seed 时，不会重复渲染），请将生成逻辑包裹在 `useAsyncData` 中：
...
HTTP API 无需安装。服务端和浏览器中的 URL 相同，因此 `<img>` 可以在任何渲染模式下工作：
