...
你可以通过 [JS-Library](/how-to-use/js-library/) 或
[HTTP-API](/how-to-use/http-api/) 在 [Next.js](https://nextjs.org/) 中使用 DiceBear。

## App Router

### 服务端组件（推荐）

在 App Router 中，组件默认是服务端组件。在服务端生成 SVG，
并将其以内联 [data URI](https://en.wikipedia.org/wiki/Data_URI_scheme) 的形式输出
—— 头像不会向客户端发送任何 JavaScript。
...
用 `'use client'` 标记文件，并将生成逻辑包裹在 `useMemo` 中，这样
只有当 seed 改变时，头像才会重新生成。
...
`idRandomization` 使用主机的非种子 RNG，因此服务端和
客户端会生成不同的 ID，React 会抛出 hydration 不匹配
警告。你可以：

- 在服务端组件中生成头像（不发生 hydration），并且不要把
  SVG 传给客户端组件，**或者**
- 保持 `idRandomization: false`，并依赖确定性的 ID。

如果你需要在同一页面上的多个头像之间保证 ID 唯一性，请将
每个头像完全在服务端渲染。
...
Pages Router 默认将每个组件都视为客户端组件。像在普通 React 应用中一样
使用 `useMemo`——请参阅
[React 指南](/guides/use-the-library-with-react/) 以了解标准
模式。通过 `getServerSideProps` 或
`getStaticProps` 进行服务端生成会将 SVG 作为 prop 返回，从而避免客户端
bundle 开销。
...
如果你想在 HTTP API 场景中使用 `next/image`，请请求栅格格式
（PNG、WebP、AVIF）——`next/image` 不会优化 SVG 源——并将
