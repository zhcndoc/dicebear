...
| `$id`        | No       | Canonical identifier for this definition (typically the URL it is hosted at) |
...
The CSS string is filtered more strictly than other attribute values. The
following are rejected:

- `@`-rules (`@import`, `@font-face`, `@keyframes`, …)
...
| `rotate`      | array  | —       | `[min, max]` rotation range in degrees (-360 to 360) |
| `scale`       | array  | —       | `[min, max]` scale factor around the component's center (0 to 10; `1` is original size) |
| `translate`   | object | —       | `{ x: [min, max], y: [min, max] }` offsets as a percentage of the component's own `width` / `height` (-1000 to 1000) |
| `variants`    | object | —       | Named variant definitions (required)           |

`translate.x` and `translate.y` are expressed as percentages of the component's own `width` and `height`. A value of `100` shifts the component by its full width or height; `0` leaves it in place. The example above (`y: [-5, 5]`) therefore means "shift the component vertically by up to 5 % of its height in either direction".

For `rotate`, `scale`, and the inner arrays of `translate`, you can also pass a single value to fix the transform instead of a `[min, max]` range.
...
is useful when you want the same visual component to appear twice (e.g. left
and right earrings) and have each occurrence pick its own variant.
...
`extends` must reference a non-alias component defined in the same `components`
map. Aliases cannot override `width`, `height`, `probability`, `rotate`,
`scale`, `translate`, or `variants` — they are pure references.
...
A handful of properties accept a `{ "type": "variable", "name": "…" }` object
in place of a literal string. Each variable is only valid in a specific spot —
the schema rejects mismatched placements.
...
| `fontFamily` | `font-family` attribute                     | Font family from options (default: `system-ui`)       |
| `fontWeight` | `font-weight` attribute                     | Font weight from options (default: `400`)             |
