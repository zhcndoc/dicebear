...
width or height; `0` leaves it in place. The example above (`y: [-5, 5]`)
therefore means "shift the component vertically by up to 5 % of its height in
either direction".

For `rotate`, `scale`, and the inner arrays of `translate`, you can also pass a
single value to fix the transform instead of a `[min, max]` range.
...
This is resolved at render time to the color selected by the PRNG for the `skin`
color group.
...
Validate definitions against the schema using the `@dicebear/schema` npm package
or any JSON Schema Draft 2020-12 validator:
