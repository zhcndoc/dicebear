...
| `$id`        | 否       | 此定义的规范标识符（通常是其托管地址的 URL）     |
| `$comment`   | 否       | 自由文本注释（例如：“Generated by Figma”）      |
...
| ------------- | ------ | ------- | ---------------------------------------------- |
| `width`       | number | —       | 组件画布宽度（必需）                            |
| `height`      | number | —       | 组件画布高度（必需）                            |
| `probability` | number | `100`   | 组件出现的概率（0-100）                        |
| `rotate`      | array  | —       | 以度为单位的 `[min, max]` 旋转范围（-360 到 360） |
| `scale`       | array  | —       | 围绕组件中心的缩放因子 `[min, max]`（0 到 10；`1` 为原始大小） |
| `translate`   | object | —       | 作为组件自身 `width` / `height` 百分比的偏移 `{ x: [min, max], y: [min, max] }`（-1000 到 1000） |
| `variants`    | object | —       | 命名的变体定义（必需）                          |

`translate.x` 和 `translate.y` 以组件自身 `width` 和 `height` 的百分比表示。值为 `100` 会使组件按其完整宽度或高度移动；`0` 则保持不动。上面的示例（`y: [-5, 5]`）因此表示“在任一方向上按组件高度的最多 5% 垂直移动组件”。

对于 `rotate`、`scale` 和 `translate` 的内部数组，你也可以传入单个值来固定变换，而不是 `[min, max]` 范围。

### Component aliases

组件可以通过 `extends` 作为另一个组件的别名。别名没有自己的
尺寸或变体——它会继承所引用组件的全部内容，但会作为一个独立的、分别随机化的实例进行渲染。这
在你希望同一个视觉组件出现两次（例如左右耳环），并让每次出现都选择自己的变体时很有用。
...
少数属性接受 `{ "type": "variable", "name": "…" }` 对象来
替代字面字符串。每个变量只在特定位置有效——
schema 会拒绝不匹配的放置位置。

| Variable     | Allowed in                                  | Description                                           |
| ------------ | ------------------------------------------- | ----------------------------------------------------- |
| `initial`    | `text` 元素 `value`                         | 从种子派生的首字母                                      |
| `initials`   | `text` 元素 `value`                         | 从种子派生的完整首字母缩写（1-2 个字符）               |
| `fontFamily` | `font-family` 属性                         | 来自选项的字体族（默认：`system-ui`）                 |
| `fontWeight` | `font-weight` 属性                         | 来自选项的字体粗细（默认：`400`）                    |
