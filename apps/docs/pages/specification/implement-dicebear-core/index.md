...
正确的实现会为相同的 seed 和样式定义生成 **逐字节完全一致的 SVG**，
与以下参考实现一致：
...
[Rust](https://github.com/dicebear/dicebear/tree/10.x/src/rust/core) 和
...
::: warning 不要在运行时计算线性化 IEEE 754 并不要求 `pow` 必须正确舍入，真实实现会在最后一位上出现分歧：V8 的 `Math.pow`、C 数学库（PHP、Python 和 Rust 使用的）、以及 Go 的纯 Go `math.Pow` 对某些通道值都会产生不同结果。若某个移植版本在运行时求值 `((s + 0.055) / 1.055) ^ 2.4`，它将无法通过某些输入上的 parity fixtures，而 JavaScript 构建甚至会在不同浏览器引擎之间表现不一致。
...
还有一个陷阱：会将 `a × b + c` 融合成单条 FMA 指令的编译器
（例如 arm64 上的 Go）只舍入一次而不是两次，从而在最后一个 ULP 上漂移。`luminance` 中的加权求和必须在每次乘法后舍入。在 Go 中，这需要围绕每个乘积显式进行 `float64(...)` 转换。 :::
...
1. 生成器注释 `<!-- Generated by DiceBear (https://dicebear.com) -->`，始终存在，并且在所有实现中保持字节级一致。
2. `<metadata>`：来自 `meta` 的 Dublin Core / RDF 块（见下文）；如果 `meta` 为空，则完全省略。
3. `<defs>`：累积的定义（clip path、gradients、组件变体主体）。实际上总是存在，因为边框圆角裁剪总会被注册。
4. `<title>`：仅在设置了 `title` 选项时存在。内容会转义。
5. 上一步中的变换后主体。
...
DiceBear 仓库提供了一套语言无关的 parity 测试套件，位于
[`tests/fixtures/parity/`](https://github.com/dicebear/dicebear/tree/10.x/tests/fixtures/parity)。
它是验证新实现的标准方式：JavaScript、PHP、Python、Rust 和 Go 的参考实现都使用同一组 JSON fixtures，并断言相同的输出，因此任何读取这些 fixtures 的移植版本都能免费获得同样的覆盖率。
