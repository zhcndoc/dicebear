---
title: Lorelei – 头像风格
description: >
  使用 DiceBear 头像库创建 Lorelei 头像。通过我们的免费头像 API 或 JavaScript 头像库生成独特的个人资料图片。
outline: [2, 3]
---

<script setup lang="ts">
import StylePreview from "@theme/components/styles/StylePreview.vue";
import StyleInfo from "@theme/components/styles/StyleInfo.vue";
import StyleDescription from "@theme/components/styles/StyleDescription.vue";
import StyleUsage from "@theme/components/styles/StyleUsage.vue";
import StyleOptions from "@theme/components/styles/StyleOptions.vue";
</script>

# Lorelei

Lorelei 是一种手绘矢量头像风格，以细致的黑色墨线呈现，
具有精细的发型、富有表现力的眼睛和素描感衣领。为社交应用、编辑类网站和用户
账户生成插画风格的 SVG 个人资料图标。

<StylePreview styleName="lorelei" />

<StyleDescription styleName="lorelei" />

## 用法

<StyleUsage styleName="lorelei" />

## 选项

<StyleOptions styleName="lorelei" />

## 详情

<StyleInfo styleName="lorelei" />
