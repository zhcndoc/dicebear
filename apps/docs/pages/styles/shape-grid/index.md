---
title: Shape Grid – 头像样式 | DiceBear
description: >
  使用 DiceBear 头像库创建 Shape Grid 头像。通过我们的免费头像 API 或 JavaScript 头像库生成独一无二
  的个人资料图片。
---

<script setup lang="ts">
import StylePreview from "@theme/components/styles/StylePreview.vue";
import StyleInfo from "@theme/components/styles/StyleInfo.vue";
import StyleDescription from "@theme/components/styles/StyleDescription.vue";
import StyleUsage from "@theme/components/styles/StyleUsage.vue";
import StyleOptions from "@theme/components/styles/StyleOptions.vue";
</script>

# Shape Grid

Shape Grid 是一种抽象的矢量头像样式，它将四个简单形状
（正方形、圆形、三角形和六边形）排列在带有色调的
背景上的 2×2 网格中。生成有趣的 SVG 个人资料图标，既可用作头像占位符，
也可用作装饰性的用户标识。

<StylePreview styleName="shape-grid" />

<StyleDescription styleName="shape-grid" />

## 用法

<StyleUsage styleName="shape-grid" />

## 选项

<StyleOptions styleName="shape-grid" />

## 详情

<StyleInfo styleName="shape-grid" />
