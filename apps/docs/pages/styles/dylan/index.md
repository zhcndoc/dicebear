---
title: Dylan – 头像风格 | DiceBear
description: >
  使用 DiceBear 头像库创建 Dylan 头像。通过我们的免费头像 API 或 JavaScript 头像库生成独特的个人资料图片。
---

<script setup lang="ts">
import StylePreview from "@theme/components/styles/StylePreview.vue";
import StyleInfo from "@theme/components/styles/StyleInfo.vue";
import StyleDescription from "@theme/components/styles/StyleDescription.vue";
import StyleUsage from "@theme/components/styles/StyleUsage.vue";
import StyleOptions from "@theme/components/styles/StyleOptions.vue";
</script>

# Dylan

Dylan 是一种大胆的、平面插画风格的矢量头像样式，具有粗线描边的
面部、手绘胡茬以及在鲜艳背景上的极简面部特征。
为创意项目和现代应用生成独特的 SVG 个人资料图标。

<StylePreview styleName="dylan" />

<StyleDescription styleName="dylan" />

## 用法

<StyleUsage styleName="dylan" />

## 选项

<StyleOptions styleName="dylan" />

## 详细信息

<StyleInfo styleName="dylan" />
